...
def compute_milp_stats(idx, n_vehicles, n_obstacles, n_pairs):
    ...
    n_init = 4 * n_vehicles
    n_term = 4 * n_vehicles
    n_obs_cons = 5 * n_vehicles * n_obstacles * horizon
...
def pack_indices(T, n_obs, n_agents, n_pairs=0):
    ...
    # Binary variables for obstacles
    idx['a'] = []
    a_cube = np.zeros((n_agents, 4, n_obs, T), dtype=int)
    for veh in range(n_agents):
        for side in range(4):
...
def build_fuel_cost(idx):
    """Implement ||u||_1 by introducing slack variables with linear constraints."""
...
def assemble_milp_data(scn):
    ...
    if enforce_sep:
        ...
        vehicle_pairs = []
    idx = pack_indices(scn['horizonSteps'], obstacles.shape[0], n_vehicles, len(vehicle_pairs))
    
    f_cost = build_fuel_cost(idx)
...
def main():
    ...
    base_scenario = {
        ...
        'posBounds': np.array([[-100, 200], [-100, 1200]]),
        'velBounds': np.array([[-50, 50], [-50, 50]]),
        'inputBounds': np.array([[-18, 18], [-18, 18]]),
        'obstacleBuffer': 0.25,
